...
class Siswa extends CI_Controller {
	...
	public function index(){
		...
		if ($this->input->post('idkelas')!="") {
			...
			$sess = array(
				'idkelas_dimenu_siswa' => $idkelas
			);
			$this->session->set_userdata( $sess );
			$idkelas = $this->session->userdata('idkelas_dimenu_siswa');
		}else{
			// echo "tidak ada";
			if ($this->session->userdata('idkelas_dimenu_siswa')!="") {
				$idkelas = $this->session->userdata('idkelas_dimenu_siswa');
			}else{
				$idkelas = "";
			}
		...
		$data['kelastetap'] = $this->db->get('kelastetap')->result();
		$data['menu1']='siswa';
		$data['siswa'] = $this->M_main->get_where('siswa','id_kelas_siswa',$idkelas)->result();
		$data['menu2']='';
		$data['content'] = 'siswa/list-siswa.php';
